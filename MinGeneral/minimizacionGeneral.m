...
for cd=12:8:28
    ...
    k=k+1;
    for N=Nmax:-1:D/dmax
        x0=[linspace(1,30,N-1),D*ones(1,N)/N];%x=[t=(t1,...,tN),d=(d1,...,dN)];
...
for cd=ceil(D/dmax):1:floor(D/dmin)
    ...
    for N=floor(D/dmin):-1:ceil(D/dmax)
        ...
        % e inferior sean nulas)
        options=optimset('TolCon',eps,'TolX', eps,'TolFun',eps,'MaxIter',1e12,'MaxFunEvals',1e12);
        [x(1:2*N-1,len-j),fval(len-j,1)]=fmincon(fun,x0,A,b,Aeq,beq,lb,ub,[],options);%el mínimo valor de la función se obtiene para x
        x(1:2*N-1,len-j)=round(x(1:2*N-1,len-j),2);%redondeo las x
        fval(len-j,1)= fun(x(1:2*N-1,len-j)')%calculo el valor de la función para la x redondeada
        if j>=1
            if fval(len-j,1)>fval(len-j+1,1)
                vecN(k)=N+1;
                eval(k)=fval(len-j,1);
                xopti(1:2*N-1,k)=x(1:2*N-1,len-j+1);
                break
            end
        end
        if j==len-1
...
toc
plot([D/dmax:1:D/dmin],vecN,'.'),xlabel('Cd (mg/L)'),ylabel('N (nº dosis)')
